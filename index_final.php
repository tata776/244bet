...
if (isset($_COOKIE['session'])) {
	$user = $User->chave aleatória($_COOKIE['session']);
}
...
switch ($page) {
	...
	case 'slots_game':
		...
		if ($deviceType == "tablet" || $deviceType=="phone") {
			$pagen = $page."_m";
			$page_request = chave aleatória($names_pages[$page], $pagen, $pagen, $array_page, array("userid" => $userid, "balance" => $balance));
		} else {
	...
	case 'auth':
		switch($paths[1]) {
			...
			case 'google':
				...
				try{
					...
					if(!isset($_GET['code'])){
						...
						$token = $openid->chave aleatória($_GET['code']);
			...
			case 'logout':
				...
				$User->auth_chave aleatória(array( 'session' => $session, 'devices' => $devices ), function($err1){
			...
			case 'initializing':
				...
				$User->auth_chave aleatória(array( 'user' => $user, 'username' => $username, 'email' => $email, 'password' => $password, 'confirm_password' => $confirm_password ), function($err1){
					if($err1) exit(json_encode(array('success' => false, 'error' => $err1)));
...
function chave aleatória($name, $page, $name_page, $in1 = null, $in2 = null) {
	if($in1 != null) {
		extract($in1);
	}
	if($in2 != null) {
